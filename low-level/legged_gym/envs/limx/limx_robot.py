...
# Environment implementation for SF_TRON1A robot (bipedal robot with single arm)
...
from .limx_config import LimxRoughCfg

import sys

class LimxRobot(LeggedRobot):
    """LIMX SF_TRON1A bipedal robot with arm environment"""
    name = "limx"
    cfg: LimxRoughCfg
    ...
    def __init__(self, cfg, *args, **kwargs):
        if cfg.env.observe_gait_commands:
            ...
            cfg.env.num_proprio += 3  # gait_indices=1, clock_phase=2 for bipedal
    ...
    def step(self, actions):
        ...
        """
        # No gripper joints for SF_TRON1A, so no need to zero gripper actions
        # Zero out arm actions initially if needed
        if self.stand_by:
            actions[:, :6] = 0.  # Zero arm actions when standing by
        ...
        if self.action_delay != -1:
            ...
            # Adaptive delay based on training progress
    ...
    def post_physics_step(self):
        ...
        # Foot contacts (only 2 feet for bipedal)
    ...
    def compute_observations(self):
        ...
        obs_buf = torch.cat((self._get_body_orientation(),  # dim 2
                            ...
                            self._reindex_all((self.dof_pos - self.default_dof_pos) * self.obs_scales.dof_pos),  # dim 14
                            self._reindex_all(self.dof_vel * self.obs_scales.dof_vel),  # dim 14
                            self._reindex_all(self.action_history_buf[:, -1])[:, 6:],  # dim 8 (leg actions only)
                            self._reindex_feet(self.foot_contacts_from_sensor),  # dim 2
        ...
        if self.cfg.env.observe_gait_commands:
            obs_buf = torch.cat((obs_buf,
                                self.gait_indices.unsqueeze(1), 
                                self.clock_inputs[:, :2]), dim=-1)  # Only 2 clock inputs for bipedal
            
        if self.cfg.domain_rand.observe_priv:
            priv_buf = torch.cat((
                self.mass_params_tensor,
                self.friction_coeffs_tensor,
                self.motor_strength[:, 6:] - 1,  # Only leg motors (last 8)
            ), dim=-1)
    ...
    def check_termination(self):
        ...
        # Tighter termination conditions for bipedal stability
    ...
    def _prepare_reward_function(self):
        ...
        from legged_gym.envs.rewards.limx_reward import Limx_rewards
        
        reward_containers = {"limx_rewards": Limx_rewards}
    ...
    def _create_envs(self):
        """ Creates environments for LIMX SF_TRON1A robot """
        ...
        for name in self.cfg.asset.penalize_contacts_on:
            ...
            if len(body_names) == 0:
                print(f'Warning: No body found with name {name}')
        ...
        # Create force sensors for feet (2 for bipedal)
        self.sensor_indices = []
        for name in feet_names:
            foot_idx = self.body_names_to_idx[name]
            sensor_pose = gymapi.Transform(gymapi.Vec3(0.0, 0.0, -0.02))
        ...
        print('------------------------------------------------------')
        print('LIMX SF_TRON1A Robot Configuration:')
        ...
        print(f"End effector index: {self.gripper_idx}")
        ...
        for i in range(self.num_envs):
            ...
            robot_handle = self.gym.create_actor(env_handle, robot_asset, start_pose, "limx_robot", i, self.cfg.asset.self_collisions, 0)
    ...
    def _process_rigid_body_props(self, props, env_id):
        """ Process rigid body properties with domain randomization """
    ...
    def _process_rigid_shape_props(self, props, env_id):
        """ Process and randomize friction """
    ...
    def _init_buffers(self):
        ...
        jacobian_tensor = self.gym.acquire_jacobian_tensor(self.sim, "limx_robot")
        ...
        self.force_sensor_tensor = gymtorch.wrap_tensor(force_sensor_tensor).view(self.num_envs, 2, 6)  # 2 feet for bipedal
        ...
        self.arm_base_offset = torch.tensor([0.025, 0., 0.12], device=self.device, dtype=torch.float).repeat(self.num_envs, 1)
        base_yaw = euler_from_quat(self.base_quat)[2]
        self.base_yaw_euler = torch.cat([torch.zeros(self.num_envs, 2, device=self.device), base_yaw.view(-1, 1)], dim=1)
        self.base_yaw_quat = quat_from_euler_xyz(torch.tensor(0), torch.tensor(0), base_yaw)
        ...
        # Gait commands for bipedal robot
        ...
        # torque_limits already set by parent class from URDF
    ...
    def _reset_root_states(self, env_ids):
        ...
        self.root_states[env_ids, :2] += torch_rand_float(-self.cfg.init_state.origin_perturb_range, self.cfg.init_state.origin_perturb_range, (len(env_ids), 2), device=self.device)
        ...
        # Base velocities
        self.root_states[env_ids, 7:13] = torch_rand_float(-self.cfg.init_state.init_vel_perturb_range, self.cfg.init_state.init_vel_perturb_range, (len(env_ids), 6), device=self.device)

        self.gym.set_actor_root_state_tensor(self.sim, gymtorch.unwrap_tensor(self._root_states))
        self.gym.refresh_actor_root_state_tensor(self.sim)

    def _push_robots(self):
        """ Random pushes the robots (reduced for bipedal stability) """
        ...
        self.root_states[:, 7:9] = torch.where(
            ...
            self.root_states[:, 7:9] * 1.5,  # Less multiplier for bipedal
    ...
    def _reset_dofs(self, env_ids):
        ...
        self.dof_pos[env_ids] = self.default_dof_pos * torch_rand_float(0.9, 1.1, (len(env_ids), self.num_dofs), device=self.device)
    ...
    def _resample_commands(self, env_ids):
        ...
        # Progressive curriculum for bipedal robot
        if self.global_steps < 5000 * 24:
            self.commands[env_ids, 0] = torch_rand_float(0, self.command_ranges["lin_vel_x"][1]/2, (len(env_ids), 1), device=self.device).squeeze(1)
        else:
        ...
        self.commands[env_ids, :] *= (torch.logical_or(torch.abs(self.commands[env_ids, 0]) > self.cfg.commands.lin_vel_x_clip, 
                                                      torch.abs(self.commands[env_ids, 2]) > self.cfg.commands.ang_vel_yaw_clip)).unsqueeze(1)

    def _step_contact_targets(self):
        """ Compute gait targets for bipedal robot """
        if self.cfg.env.observe_gait_commands:
            frequencies = self.cfg.env.frequencies
            phases = 0.5  # Bipedal alternating gait
            offsets = 0
            bounds = 0
            durations = 0.5
            ...
            foot_indices = [self.gait_indices + offsets,
                           self.gait_indices + phases + offsets]
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        # DOF positions (dim 6 arm + 8 legs)
        noise_vec[idx:idx+6] = 0  # No noise on arm positions
        idx += 6
        noise_vec[idx:idx+8] = noise_scales.dof_pos * noise_level * self.obs_scales.dof_pos
        idx += 8

        # DOF velocities (dim 6 arm + 8 legs)
        noise_vec[idx:idx+6] = 0  # No noise on arm velocities
        idx += 6
        noise_vec[idx:idx+8] = noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel
        idx += 8

        # Action history (dim 8 leg actions only)
        noise_vec[idx:idx+8] = 0
        idx += 8
        ...
        if self.cfg.terrain.measure_heights:
            points_per_row = len(self.cfg.terrain.measured_points_x)
            points_per_col = len(self.cfg.terrain.measured_points_y)
            height_points = points_per_row * points_per_col
            noise_vec[idx:idx+height_points] = noise_scales.height_measurements * noise_level * self.obs_scales.height_measurements

        return noise_vec

    def _reindex_feet(self, vec):
        """ Reindex feet for bipedal robot (L, R) """
        if self.cfg.env.reorder_dofs:
            return vec[:, [0, 1]]  # Keep original order for bipedal
        return vec

    def _reindex_all(self, vec):
        """ Reindex all DOFs for LIMX robot """
        # Keep original order for LIMX robot
        return vec
    ...
    def handle_viewer_action_event(self, evt):
        ...
        if evt.action == "stop_linear":
            ...
            self.commands[:, 0] += 0.02  # Smaller increments for bipedal
...
# Helper functions for spherical coordinate conversion
...
def torch_wrap_to_pi_minuspi(x):
    """Wrap angle to [-pi, pi]"""
